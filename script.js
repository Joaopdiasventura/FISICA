const maxRadius = 200;
        ...
        const setupGUI = () => {
            ...
            configFolder.add(settings, 'numColors', 1, 7, 1).name('NÚMERO DE CORES')
            ...
            configFolder.add(settings.atoms, 'count', 1, 1000, 1).name('ATOMOS POR COR').listen().onFinishChange(v => {
                randomAtoms(v, true)
            })
            configFolder.add(settings, 'time_scale', 0.1, 5, 0.01).name('TEMPO').listen()
            configFolder.add(settings, 'viscosity', 0.1, 2, 0.1).name('VISCOSIDADE').listen()
